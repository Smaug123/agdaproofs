...
swapGo'' : (a b : BinNat) → go<B FirstGreater a b ≡ FirstGreater → go<B FirstLess b a ≡ FirstLess
swapGo' : (a b : BinNat) → go<B FirstLess a b ≡ FirstGreater → go<B FirstGreater b a ≡ FirstLess

swapGo'' [] [] a<b = refl
swapGo'' [] (zero :: b) a<b = swapGo'' [] b a<b
swapGo'' (zero :: a) [] a<b = refl
swapGo'' (zero :: a) (zero :: b) a<b = swapGo'' a b a<b
swapGo'' (zero :: a) (one :: b) a<b = swapGo' a b a<b
swapGo'' (one :: a) [] a<b = refl
swapGo'' (one :: a) (zero :: b) a<b = swapGo'' a b a<b
swapGo'' (one :: a) (one :: b) a<b = swapGo'' a b a<b

swapGo' (zero :: a) [] a<b = swapGo' a [] a<b
swapGo' (zero :: a) (zero :: b) a<b = swapGo' a b a<b
swapGo' (zero :: a) (one :: b) a<b = swapGo' a b a<b
swapGo' (one :: a) [] a<b = refl
swapGo' (one :: a) (zero :: b) a<b = swapGo'' a b a<b
...
swapGo : (a b : BinNat) → go<B Equal a b ≡ FirstGreater → go<B Equal b a ≡ FirstLess
swapGo [] (zero :: b) a<b = swapGo [] b a<b
swapGo (zero :: a) [] a<b = swapGo a [] a<b
swapGo (zero :: a) (zero :: b) a<b = swapGo a b a<b
swapGo (zero :: a) (one :: b) a<b = swapGo' a b a<b
...
PartialOrder._<_ BinNatOrder b1 b2 = (b1 <B b2) ≡ FirstLess
PartialOrder.irreflexive BinNatOrder {x} pr = bad
  where
    bad : False
    bad with transitivity (<BIsInherited x x) pr
    ... | p with TotalOrder.totality ℕTotalOrder (binNatToN x) (binNatToN x)
    bad | p | inl (inl x) = TotalOrder.irreflexive ℕTotalOrder x
    bad | p | inl (inr x) = TotalOrder.irreflexive ℕTotalOrder x
    bad | p | inr x = badCompare p
PartialOrder.<Transitive BinNatOrder {x} {y} {z} a<b b<c = v
  where
    v : (x <B z) ≡ FirstLess
    v with transitivity (<BIsInherited x y) a<b
    v | t with transitivity (<BIsInherited y z) b<c
    v | t | u with TotalOrder.totality ℕTotalOrder (binNatToN x) (binNatToN y)
    v | _ | u | inl (inl x<y) with TotalOrder.totality ℕTotalOrder (binNatToN y) (binNatToN z)
    v | _ | _ | inl (inl x<y) | inl (inl y<z) = transitivity (equalityCommutative (<BIsInherited x z)) g
      where
        x<z : binNatToN x <N binNatToN z
        x<z = TotalOrder.<Transitive ℕTotalOrder x<y y<z
        g : (x <BInherited z) ≡ FirstLess
        g with TotalOrder.totality ℕTotalOrder (binNatToN x) (binNatToN z)
        g | inl (inl x) = refl
        g | inl (inr z<x) = exFalso (TotalOrder.irreflexive ℕTotalOrder (TotalOrder.<Transitive ℕTotalOrder x<z z<x))
        g | inr x=z = exFalso (TotalOrder.irreflexive ℕTotalOrder (TotalOrder.<WellDefined ℕTotalOrder x=z refl x<z))

BinNatPOrder : SetoidPartialOrder Canonical (PartialOrder._<_ BinNatOrder)
SetoidPartialOrder.<WellDefined BinNatPOrder {a} {b} {c} {d} a=b c=d a<c = transitivity (transitivity (canonicalFirst b d Equal) (transitivity (canonicalSecond (canonical b) d Equal) (transitivity (applyEquality2 (go<B Equal) (equalityCommutative a=b) (equalityCommutative c=d)) (equalityCommutative (transitivity (canonicalFirst a c Equal) (canonicalSecond (canonical a) c Equal)))))) a<c
SetoidPartialOrder.irreflexive BinNatPOrder {a} a<a = PartialOrder.irreflexive BinNatOrder {a} a<a
SetoidPartialOrder.<Transitive BinNatPOrder {a} {b} {c} a<b b<c = PartialOrder.<Transitive BinNatOrder {a} {b} {c} a<b b<c

BinNatTOrder : SetoidTotalOrder BinNatPOrder
SetoidTotalOrder.totality BinNatTOrder a b with inspect (a <B b)
SetoidTotalOrder.totality BinNatTOrder a b | Equal with≡ pr = inr (comparisonEqual a b pr)
SetoidTotalOrder.totality BinNatTOrder a b | FirstLess with≡ pr rewrite pr = inl (inl refl)
SetoidTotalOrder.totality BinNatTOrder a b | FirstGreater with≡ pr = inl (inr (swapGo a b pr))

ordersAgree' : (a b : BinNat) → PartialOrder._<_ BinNatOrder a b → (binNatToN a) <N (binNatToN b)
ordersAgree' a b a<b with <BIsInherited a b
... | inh with TotalOrder.totality ℕTotalOrder (binNatToN a) (binNatToN b)
ordersAgree' a b a<b | inh | inl (inl x) rewrite inh = x
ordersAgree' a b a<b | inh | inl (inr x) rewrite a<b = exFalso (badCompare'' (equalityCommutative inh))
ordersAgree' a b a<b | inh | inr x rewrite equalityCommutative inh = exFalso (badCompare a<b)

ordersAgree : (a b : ℕ) → a <N b → PartialOrder._<_ BinNatOrder (NToBinNat a) (NToBinNat b)
ordersAgree a b a<b with SetoidTotalOrder.totality BinNatTOrder (NToBinNat a) (NToBinNat b)
ordersAgree a b a<b | inl (inl x) = x
ordersAgree a b a<b | inl (inr x) with ordersAgree' (NToBinNat b) (NToBinNat a) x
... | bl rewrite nToN b | nToN a = exFalso (TotalOrder.irreflexive ℕTotalOrder (TotalOrder.<Transitive ℕTotalOrder bl a<b))
ordersAgree a b a<b | inr x rewrite NToBinNatInj a b x = exFalso (TotalOrder.irreflexive ℕTotalOrder a<b)
